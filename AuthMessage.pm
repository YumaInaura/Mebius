...
my(%init) = &InitMessage();
my(%to_renew,%from_renew,$view_line,$plustype_message_view,$plustype_check_friend_from,$plustype_check_friend_to,%renew_option,%renew_account);
...
# 新しいメッセージIDを定義
my($to_message_id) = Mebius::Char(undef,30);

# 新しいメッセージIDを定義
my($from_message_id) = Mebius::Char(undef,30);
...
my $file = "${directory2}${message_id}_message.dat";
	...
	if($main::time < $message{'sendtime'} + 3*24*60*60){
		$message{'new_flag'} = 1;
	}
	...
	# 開封する場合
	if($type =~ /Open-message/ && $message{'to_account'} eq $main::myaccount{'file'} && !$message{'open_time'}){
		my(%renew_box);
		$message{'renew_flag'} = 1;
		$message{'open_time'} = $main::time;
		$type .= qq( Renew);
...
my $directory = "${account_directory}message/";
	...
	$box{'new_message'} = 0;
		...
		if($type =~ /Get-new-status/ && $main::time >= $box{'lastcatch_time'} + 3*24*60*60){
...
my($type,$account) = @_;
my(%init) = &InitMessage();
my($selected_flag,$delete_message_num,%boxtype,%boxtype_opened_message);
...
# ディレクトリ / ファイル定義
my $directory1 = "${main::int_dir}_authlog/";
my $file = "${directory1}message_history.log";
