...
use Mebius::Mixi::Submit::OutComment;
...
my $data_group = $self->fetchrow_main_table_desc({ },"create_time",{ LIMIT => 100 });
...
my $fetch = $event->useful_sql($topic_type);
...
my $basic = $self->basic_object();
my $param  = $query->param();
my(%input,@message,$succeed_flag,$event_kind_number,$event_edit_flag);
...
my $submit_data = my $submit_data_exists = $submited_data_group->[0];
	...
	if($submit_data_exists && !$console->option("all") && !$param->{'all'}){
			...
			if($topic_type eq "topic"){
					...
					} elsif(!$submit_data->{'event_deleted_flag'}) {
...
my $account_data = my $account_data_exists = $mixi_account->useful_account_data_and_try();
...
my($title,$bbs_body) = $event->order_title_and_body($event_data);
my $order = $event->order();
...
my $mixi_event_id = $self->submited_html_to_event_id($submit_html);
	...
	if($mixi_event_id){
		...
		$self->insert_main_table({ event_target => $event_target , email => $email , account => $account , submit_type => "post" , topic_type => $topic_type , mixi_community_id => $community_id , mixi_event_id => $mixi_event_id , event_kind_number => $event_kind_number , title => $title , bbs_body => $bbs_body , event_order => $order });

		$event->update_main_table_where({ last_order => $order },{ target => $event_target });
...
submit_type => "comment" ,
mixi_community_id => $mixi_community_id ,
mixi_event_id => $mixi_event_id ,
comment_deleted_flag => 0
);
...
console "Try to delete $num comments on  $topic_type.";
	...
	foreach my $submit_data (@{$before_comment_data_group}){
		my $email = $submit_data->{'email'} || (warn("Email is empty.") && next);
...
$print .= $html->href($event_url) . "\n";
	...
	if(!$param->{'view_only_url'}){
		...
		$print .= $times->how_before($data->{'last_check_alive_time'}) . "\n";
			...
			if($data->{'last_edit_time'}){
				$print .= " (編集 " . $times->how_before($data->{'last_edit_time'}) . " )\n";
			}
