...
Mebius::Template::gzip_and_print_all({},$all_line);
...
my($thread) = Mebius::BBS::thread({ TypeGetHashDetail => 1 , ReturnReference => 1 },$realmoto,$thread_number);
	...
	# ●正常に過去ログメニューが作られていない分を修正
	if($type =~ /Fix-2012\.08\.29\.15/){
		$renew{'.'}{'concept'} = " Fix-2012.08.29.15";
		$renew{'bepast_time'} = $thread->{'bepast_time'}; # とりあえず代入
		my $add_line = qq($thread->{'number'}<>$thread->{'sub'}<>$thread->{'res'}<>$thread->{'posthandle'}<>$thread->{'lastrestime'}<>$thread->{'lasthandle'}<>1<>\n);
		Mebius::BBS::old_type_past_menu_file({ Renew => 1 , bbs_kind => $realmoto ,  add_line => [$add_line] , thread_number => $thread->{'number'} });
	}
...
# 記事を更新
my($renewed_thread) = Mebius::BBS::thread({ Renew => 1 , ReturnReference => 1 , select_renew => \%renew },$realmoto,$thread_number);
...
my($line_num_all,$pagelinks_number,$search_keyword_encoded,%postbuf,%data);
	...
	# ディレクトリ定義
	#if(! my = $directory ){
	#	die("Can't decide file name.");
	#}

my($directory1) = Mebius::BBS::past_index_directory_path_per_bbs($realmoto);
...
#my $directory2 = "${directory1}_${realmoto}_index/";
	...
	foreach $file_split (@files){
		...
		# ファイル定義
		if($file_split =~ /^(\w+)$/){ $file = "${directory1}${realmoto}_${file_split}_past.log"; } else { next; }
...
#my $directory1 = "${main::int_dir}_bbs_index/";
#my $directory2 = "${directory1}_${realmoto}_index/";
my $file = "${directory1}${realmoto}_allindex.log";

	# ファイルを開く
	my($past_allindex_handler,$read_write) = Mebius::File::read_write($type,$file,[$directory1]);
...
# 旧過去ログ処理
#-----------------------------------------------------------
sub old_type_past_menu_file{

# 宣言
my($use,$select_renew) = @_;
my($i,@renew_line,%self,$FILE1,$renew,%data_format,%self_renew,$file1);
...
my($web_data_directory) = Mebius::BaseInitDirectory();
	...
	# 掲示板の指定がない場合
	if($use->{'bbs_kind'} eq ""){
		warn("Perl warn! bbs_kind ( $use->{'bbs_kind'} ) is empty.");
		return();
	}

#my $directory1 = $self{'directory1'} = "${web_data_directory}";
my($file1) = Mebius::BBS::old_type_past_file_path($use->{'bbs_kind'});
	...
	# リターン処理
	if($use->{'Renew'}){
			if(!$use->{'add_line'}){ return(); }
			if(defined $use->{'add_line'} && ref $use->{'add_line'} ne "ARRAY"){ die("Perl Die! $use->{'add_line'} is not ARRAY HASH."); }
	}
...
#$data_format{'1'} = [('key')];
	...
	while(<$FILE1>){
		...
		# この行を分解
		chomp;
		my($thread_number2,@another_data) = split(/<>/);

			# 更新用
			if($use->{'Renew'}){

					# 重複分
					if($use->{'thread_number'} && $thread_number2 == $use->{'thread_number'}){
						next;
					}
				...
				# 行を追加
				push(@renew_line,Mebius::add_line_for_file([$thread_number2,@another_data]));
			}
	...
	if($use->{'Renew'}){
			...
			# 指定された過去ログ行を直接的に追加
			if(@{$use->{'add_line'}}){
				unshift(@renew_line,@{$use->{'add_line'}});
			}

		# 任意の更新とリファレンス化
		my($renew) = Mebius::Hash::control(\%self,$select_renew,\%self_renew);
