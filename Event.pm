...
$print .= qq(<div>);
$print .= qq(<textarea cols="75" rows="15" name="bbs_body" style="width:60%;height:40em;" placeholder="新規投稿">).e($data->{"bbs_body"}).qq(</textarea>);
$print .= qq(</div>);

$print .= qq(<div>);
$print .= qq(<textarea cols="75" rows="15" name="bbs_body" style="width:60%;height:20em;" placeholder="コメント">).e($data->{"bbs_comment"}).qq(</textarea>);
$print .= qq(</div>);
...
$print .= $self->contact_accounts_select_box($data);
	...
	if( my $contact_account = $data->{'contact_account'}){
		$print .= $html->href("http://mixi.jp/show_profile.pl?id=$contact_account","[mixi]");
	}
...
$insert{'bbs_comment'} = $param->{'bbs_comment'};
...
$title =~ s!\[schedule\]!$event_data->{'schedule_body'}!gie;
	...
	if( my $mixi_account_number = $event_data->{"contact_account"} ){
		my $mixi_account_url = "http://mixi.jp/show_profile.pl?id=$mixi_account_number";
