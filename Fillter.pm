...
$text_for_judge = $self->delete_marks($text_for_judge);
	...
	if($text_for_judge =~ /(ドピュ|どぴゅ|性器|精子|射精|膣|陰核|陰茎|夢精|幼女|ようじょ|自慰|勃起|ぼっき|ボッキ|乳首|巨乳|貧乳|勃起|処女|童貞|性器|陰毛|お(っ)?ぱい|オ(ッ)?パイ)/s){ $fillter_flag = 1; }
	if($text_for_judge =~ /(セフレ|ｾﾌﾚ|(ヴァ|バ)ージン|セックス|せっくす|ｾｯｸｽ|(S|ｓ|Ｓ)(E|ｅ|Ｅ)(X|ｘ|Ｘ|×)|セクロス|ｾｸﾛｽ|(オナ|ｵﾅ)(禁|ニ|ヌ)?|えっち|(工|エ)ッチ|ｴｯﾁ|ペニス|ﾍﾟﾆｽ|ロリコン|ﾛﾘｺﾝ|コンドーム|ポルノ|性交|交尾|亀頭|アナル|ｱﾅﾙ|クンニ|フェ(ラ|○|●|♯|□|※|＊|＃)|ﾌｪﾗ|イマラチオ|ヤリマン|マン汁|ヤリチン|エクスタシ|ｴｸｽﾀｼ|マスターベーション|ちんちん|チンチン|ﾁﾝﾁﾝ)/i){ $fillter_flag = 1; }

	if($text_for_judge =~ /(淫乱|乱交|スケベ|ネクロフィリア|姦|ワレメ|欲求不満|体位|ツルペタ|手マン|子宮|卑猥|性欲|露出|監禁|金玉|睾丸|手淫|大便|精液|中出し|生出し|立(ち)?バック|騎乗位|愛液)/s){ $fillter_flag = 1; }
	...
	if($text_for_judge =~ /(ク|ｸ|く)(リ|ﾘ|り|○|●|♯|□|※|＊)(ト|ﾄ|と|○|●|♯|□|※|＊)(リ|ﾘ|り|○|●|♯|□|※|＊)(ス|ｽ|す)/s){ $fillter_flag = 1; }
	if($text_for_judge =~ /(エ|ｴ|え)(ツ|ﾂ|つ|っ)(チ|ﾁ|ち)/s){ $fillter_flag = 1; }

	if($text_for_judge =~ /(^|[^A-Za-z])(H|Ｈ)(する|すれ|した|しま|しよ|して|しな)/s){ $fillter_flag = 1; }
	if($text_for_judge =~ /(アソコ)(.*)(毛)/s){ $fillter_flag = 1; }

	if($text_for_judge =~ /(.*)(毛)/s){ $fillter_flag = 1; }
	if($text_for_judge =~ /(リスカ|アムカ|リストカット|アームカット)/s){ $fillter_flag = 1; }
