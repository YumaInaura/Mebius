...
my($FILE1,$read_write) = Mebius::File::read_write($type,$file);
	...
	foreach (@log_line) {
		...
		# この行を分解
		chomp;
		my($key2,$account2,$handle2,$intro2,$befriend_time2,$edit_time2,$last_edit_time2,$be_introduction_comment2,$last_access_time2,$last_get_account_data_time2,$account2_friend_num2) = split(/<>/);
			...
			if($type =~ /Get-all-index/){
				...
				$self{'topics_line'} .= qq(<a href="${main::auth_url}$account2/">$handle2</a> );
					...
					if($hit_all_index <= 7){
						...
						$be_introduction_comment2 =~ s/<br>/ /g;
		...
		Mebius::Auth::FriendsFriendIndex("New-cancel Renew",$account2,$account{'file'},$other_friend{'file'});
			...
			if($type =~ /Renew/){
					...
					my($allow_renew_line_flag) = Mebius::RenewStatus::allow_judge_for_get_file($last_get_account_data_time2,$last_access_time2);
					...
					# ▼紹介文の変更
					if($type =~ /Change-be-introduction/){
							if($account2 eq $renew{'account'}){
								$be_introduction_comment2 = $renew{'be_intro'};
							}
					}
					...
					# 更新行を追加する
					push(@renew_line,"$key2<>$account2<>$handle2<>$intro2<>$befriend_time2<>$edit_time2<>$last_edit_time2<>$be_introduction_comment2<>$last_access_time2<>$last_get_account_data_time2<>$account2_friend_num2<>\n");
			}
			...
			if($index_line){
				$self{'index_line'} = qq(<h2 id="MYMEBI"$main::kstyle_h2>$main::friend_tag ($hit_index人)</h2><div class="line-height-large friend_index">$index_line</div>);
					if($type =~ /Get-friend-status/ && $self{'last_renew_status_time'}){
						my($how_long) = Mebius::second_to_howlong({ GetLevel => "top" , ColorView => 1 , HowBefore => 1 },time - $self{'last_renew_status_time'});
...
close($FILE1);
	...
	# バックアップ
	if($type =~ /Renew/ && (rand(25) < 1 || Mebius::AlocalJudge())){
...
my(%apply,$apply_index_handler,@renew_index,$index_line,$i,$hit_index);
my $time = time;
...
# 局所化
my($type,$account,$diary_account,$diary_number,$diary_subject,$diary_handle) = @_;
		...
		my($key2,$account2,$handle2,$diary_number2,$subject2,$posttime2,$postdate2) = split(/<>/);
			...
			if($type =~ /Get-topics/){
				...
				my($post_before2) = Mebius::SplitTime("Get-top-unit Color-view Plus-text-前",$main::time - $posttime2);
					...
					if($hit_topics >= 4){ $topics_flow_flag = 1; last; }
...
close($friend_diary_index);
	...
	if($type =~ /Get-topics/){
			...
			# 見出しの整形
			$topics_line = qq(<h3$main::kstyle_h3><a href="friend-diary">マイメビの新着日記</a></h3><div class="mdiary line-height-large">$topics_line</div>);
