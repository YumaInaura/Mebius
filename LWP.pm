...
my $failed_flag = $proxy->html_to_failed_flag($html);

	if($failed_flag){
		$proxy->update_main_table({ failed_count => ["+",1] },{ WHERE => { proxy => $use->{'proxy'}  } });
	} else {
		$proxy->update_main_table({ succeed_count => ["+",1] },{ WHERE => { proxy => $use->{'proxy'}  } });
	}
	...
	if($http_proxy){
			...
			if($use->{'url'} =~ /^https/){
					if(Mebius::alocal_judge()){
						...
						$ua->proxy('https', "connect://$proxy_addr_and_port/");
					}

			} else {
				$ua->proxy([qw(http https)], $http_proxy );
			}
		console "Use proxy $http_proxy ";
	}
