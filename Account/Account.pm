...
my $self = shift;
$self->account_data_to_one_action_plus(@_);
...
my @text = split(/\n|\r/,$all_of_text);
	...
	foreach my $text (@text){
		...
		my($email,$password,$account_number,$nickname) = split(/[\t\s]+/,$text);
		...
		push @account , { email => $email , password => $password , account => $account_number , nickname => $nickname };
	}
...
console || die;
	...
	if($console->option('-reserve_accounts')){
		...
		$data_group = $self->fetchrow_main_table({ account_type => ["<>","special"] , last_failed_time => ["<",time-1*24*60*60] , last_login_missed_time => ["<",time-7*24*60*60] , status => ["IS","NULL"] },{ Debug => 0 });
	}
...
my $moved_url = "http://mixi.jp/show_profile.pl?id=$acocunt_id";
