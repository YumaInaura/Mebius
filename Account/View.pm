...
$print .= $self->links();
	...
	if($param->{'account_type'} eq "useful" || $param->{'account_type'} eq "all"){
		...
		$print .= $mixi_account->data_group_to_table($data_group,"利用可能なアカウント",{ Password => $param->{'view_password'} });
	}
	...
	if($use->{'Password'}){
		...
		my $span_title = "$data->{'password'} $data->{'year'}/$data->{'month'}/$data->{'day'}";
			...
			if($param->{'edit'}){
				...
				$print .= $html->href("http://mixi.jp/show_profile.pl?id=$data->{'account'}",$data->{'account'},{ title => $span_title });
			} else {
		...
		$print .= qq(</td>);
			...
			if($param->{'edit'}){
				$print .= qq(<td>);
					if($data->{'account'}){
						$print .= $html->href("http://mixi.jp/show_profile.pl?id=$data->{'account'}",$data->{'account'});
					} else {
