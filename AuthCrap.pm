			...
			if($type =~ /Get-topics/ && $i <= 10){
				$topics_line .= qq(<a href="${main::auth_url}$account2/">$handle2</a>\n);
					...
					if($main::myaccount{'file'} eq $account2 || $main::myaccount{'file'} eq $account || $main::myadmin_flag >= 1){
						$topics_line .= qq( (<a href="${main::auth_url}$account/?mode=crap&amp;mode=crap&amp;action=delete_crap&amp;target=diary&amp;diary_number=$file_number&target_account=$account2&amp;account_char=$main::myaccount{'char'}">削除</a>)\n);
					}
			}
...
$data{'topics_line'} = $topics_line;
	...
	if($type =~ /New-crap/){
			...
			# 重複拍手の場合
			if($data{'craped_flag'} && !$main::alocal_mode){ main::error("既に拍手しています。"); }
